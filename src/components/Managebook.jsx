  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[rgba(15,23,42,0.55)]">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 sm:px-6 pt-5 sm:pt-6 pb-4 border-b border-slate-100">
          <h2 className="text-base sm:text-lg font-bold text-slate-800">Edit Book</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <label className="flex flex-col gap-1">
              ...
              <input type="number" step="0.01" value={rating} onChange={(e) => setRating(e.target.value)} required
                className="border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-400 transition" />
            </label>
          ...
          <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-2">
            <button type="button" onClick={onClose}
              className="w-full sm:w-auto px-5 py-2 rounded-xl text-sm font-semibold text-slate-600 border border-slate-200 hover:bg-slate-50 transition">
              Cancel
            </button>
            <button type="submit" disabled={loading}
              className="w-full sm:w-auto px-5 py-2 rounded-xl text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 transition">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[rgba(15,23,42,0.55)]">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-5 sm:p-6 space-y-4">
        ...
        <h2 className="text-center text-base sm:text-lg font-bold text-slate-800">Delete Book?</h2>
    ...
    <div className="min-h-screen bg-slate-50 font-[Poppins] p-4 sm:p-8">
      <div className="max-w-6xl mx-auto space-y-5 sm:space-y-7">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <Link to="/addbooks">
            <button className="flex items-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 rounded-2xl text-white text-sm font-semibold shadow-md hover:-translate-y-0.5 transition-transform bg-purple-500">
              + Add New Book
            </button>
          </Link>
        </div>
                  ...
                  <thead>
                    <tr className="border-b border-slate-100">
                      {["Book", "Author", "Category", "Price", "Stock", "Rating", "Actions"].map((h) => (
                        <th key={h} className="text-left px-5 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wide">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {filteredBooks.length === 0 && (
                      <tr>
                        <td colSpan={7} className="text-center py-16 text-slate-400 text-sm">No books found.</td>
                      </tr>
                    )}
                    {filteredBooks.map(function (book) {
                      return (
                        <tr key={book._id} className="border-b border-slate-50 hover:bg-slate-50/70 transition-colors">
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-3">
                              <img src={book.image} alt={book.title}
                                className="w-10 rounded-lg shadow-sm shrink-0 object-cover"
                                style={{ height: "52px" }}
                          ...
                          <td className="px-5 py-4">
                            <span className="flex items-center gap-1 text-amber-500 font-semibold">★ {book.rating}</span>
                          </td>
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-2">
                              ...
                              <button onClick={() => setDeleteBook(book)}
                                className="p-2 rounded-xl text-red-400 hover:bg-red-50 transition-colors" title="Delete">
                  ...
                  return (
                    <div key={book._id} className="flex items-start gap-3 p-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-slate-800 text-sm leading-tight truncate">{book.title}</p>
                        <p className="text-xs text-slate-500 mt-0.5">{book.author}</p>
                        <div className="flex flex-wrap items-center gap-2 mt-2">
                          <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full ${getCategoryClass(book.category)}`}>
                            {book.category}
                          </span>
